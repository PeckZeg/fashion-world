...
const createClientAsync = reqlib('./redis/createClientAsync');
...
const cacheKey = 'scripts:sync-video:restore-definitions';
const OPTS = { new: true };

module.exports = (ftpVideo, connectOpts) => createClientAsync()

  // check video is handled
  .then(client => (
    client.sismemberAsync(cacheKey, ftpVideo.sha1)
      .then(exists => ({ client, exists }))
  ))
  ...
  .then(({ client, exists }) => {
    ...
    return SourceVideo.findOne({ sha1: ftpVideo.sha1 }).then(source => {
      ...
      return syncUtils.ftp.create()
        ...
        // add to handled queue
        .then(filename => (
          client.saddAsync(cacheKey, ftpVideo.sha1)
  ...
  // close redis client
  .then(({ client, filename }) => (
    client.quitAsync().then(() => filename)
  ));
