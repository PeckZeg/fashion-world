...
export default class Simiaogongfo extends Component {
    static contextTypes = {
    ...
    componentDidMount() {
        ...
        var _this = this;
        Proxy.get(`http://cms.sdwhcn.com/api/temple/buddha${this.props.location.search}`, function (data) {
            // var listh = [{}];
    ...
    onClickcell = data => {
        console.log(data);
        this.context.router.push({ pathname: 'gaoshengdetail', query: { id: data.id,tag:'buddha' } });
    }
    render() {
        return (
            <LoadingLayout visible={this.state.loadingVisible}>
                <WhiteSpace size="sm" style={{ backgroundColor: '#f8f8f8' }} />
                <Gongfolist tag='gongfo' newslist={this.state.newsdata} nextpageurl={this.state.nextpageurl} oncardclick={this.onClickcell} />
            </LoadingLayout>
        );
    }
}
