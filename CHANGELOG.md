...
* 跨域白名单新增域名：`http://beta.cms.fashionworldcn.com`
* 修正了视频有时候无法注入分类的问题。
* 客户端接口
  * 视频
    * `GET` 获取视频列表新增 `priority` 排序值
    * 新增 `GET` 获取多重视频列表
* 后台接口
  * 视频
    * `PUT` 更新视频新增 `priority` 编辑项
...
* 客户端接口
  * 个人中心
    * 新增 `POST` 第三方账号注册
    * 修正 `GET` 获取当前用户喜欢的视频列表 不能正常访问的问题
* 后台接口
  * 修正了带有 `sortCreateAt` 参数的接口不能正常起作用的问题
