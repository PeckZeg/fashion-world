...
const ACTION = config.apiActions['client:user:destroy-personal-favourite-videos'];
  ...
  authToken(ACTION, req.header('authorization'))
    ...
    .then(({ client, videoIds, userId }) => {
      ...
      return client.hkeysAsync(
        keys('client:user:favourite-videos')(userId)
      )
    ...
    .then(({ client, multi, videoIds, userId }) => {
      videoIds.forEach(videoId => {
        ...
        multi.hdel(
          keys('client:video:favourite-users')(videoId),
          userId.toString()
        );
      });
